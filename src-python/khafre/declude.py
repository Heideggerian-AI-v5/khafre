...
class Decluder(TaskableProcess):
    def __init__(self):
        super().__init__()
    ...
    def _performStep(self):
        ...
        for name, maskData in masks.items():
            ...
            if (name not in self._polygonData):
                ...
                maskData["maskSoll"] = maskData["maskIst"]
            else:
                polyTarget = numpy.concatenate(maskData["polygons"], axis=0)
                polyStart = numpy.concatenate(self._polygonData[name]["polygons"], axis=0)
